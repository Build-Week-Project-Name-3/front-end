import React from 'react';

const Field = React.forwardRef(({label, type}, ref) => {
    return (
      <div>
        <label style={labelStyle} >{label}</label>
        <input ref={ref} type={type} style={inputStyle} />
      </div>
    );
});

export default function AddPlantForm(props)  {
    const submit = props.onSubmit;
    const plantNameRef = React.useRef();
    const imageUrlRef = React.useRef();
    const h20FrequencyRef = React.useRef();
    const plantSpeciesRef = React.useRef();
    const handleSubmit = e => {
        e.preventDefault();
        const data = {
            plant_name: plantNameRef.current.value,
            image_url : imageUrlRef.current.value,
            h20Frequency: h20FrequencyRef.current.value,
            plant_species: plantSpeciesRef.current.value
        };
        submit(data);
    };
    return (
      <form style={formStyle} onSubmit={handleSubmit} >
        <h2>Add Plant</h2>
        <Field ref={plantNameRef} label="Plant Name:" type="text" />
        <Field ref={imageUrlRef} label="Image Url" type="text" />
        <Field ref={h20FrequencyRef} label="Watering Frequency (Days)" type="text" />
        <Field ref={plantSpeciesRef} label="Plant Species" type="text" />
        <div>
          <button style={submitStyle} type="submit">Submit</button>
        </div>
      </form>
    );
};


const formStyle = {
  margin: 'auto',
  padding: '10px',
  border: '1px solid #c9c9c9',
  borderRadius: '5px',
  background: '#f5f5f5',
  width: '220px',
  display: 'block'
};

const labelStyle = {
  margin: '10px 0 5px 0',
  fontFamily: 'Arial, Helvetica, sans-serif',
  fontSize: '15px',
};

const inputStyle = {
  margin: '5px 0 10px 0',
  padding: '5px', 
  border: '1px solid #bfbfbf',
  borderRadius: '3px',
  boxSizing: 'border-box',
  width: '100%'
};

const submitStyle = {
  margin: '10px 0 0 0',
  padding: '7px 10px',
  border: '1px solid #efffff',
  borderRadius: '3px',
  background: '#3085d6',
  width: '100%', 
  fontSize: '15px',
  color: 'white',
  display: 'block'
};